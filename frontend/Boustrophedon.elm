...
port textContentRunner = S.map (\x -> (if | x == ""   -> Task.succeed ""
                                          | otherwise -> Http.getString (Server.serverUrl ++ "texts/"  ++ x))
...
port menuAction =
    let toMenuAction tap viewDims =
        if | tap.y == 0                             -> Signal.send fileName.address ""
           | tap.y > (viewDims.fullHeight // 5) &&
